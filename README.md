...
[![Live Demo](https://img.shields.io/badge/Live_Demo-Streamlit-FF4B4B?logo=streamlit&logoColor=white)](https://sec-filings-rag-intelligence.streamlit.app/)
[![Live API](https://img.shields.io/badge/Live_API-Render-46E3B7?logo=render&logoColor=white)](https://sec-filings-rag-api.onrender.com/docs)
[![API Health](https://img.shields.io/badge/API_Health-%2Fhealth-success)](https://sec-filings-rag-api.onrender.com/health)

**Live Demo:** [sec-filings-rag-intelligence.streamlit.app](https://sec-filings-rag-intelligence.streamlit.app/)  
**API Docs:** [sec-filings-rag-api.onrender.com/docs](https://sec-filings-rag-api.onrender.com/docs)  
**API Health:** [sec-filings-rag-api.onrender.com/health](https://sec-filings-rag-api.onrender.com/health)
...
This project is designed as a portfolio-grade example of **RAG engineering, information retrieval, financial data processing, backend/API design, evaluation, observability, testing, containerization, and cloud deployment**—not as a notebook-only chatbot.

A public portfolio demo is deployed with **Streamlit Community Cloud** as the research UI and a **Render-hosted FastAPI backend** for SEC/XBRL access and routed research requests.
...
| Public demo | Streamlit Community Cloud frontend + Render FastAPI backend |
...
> The backend/package dependency source of truth is `pyproject.toml`. `app/requirements.txt` is intentionally lightweight for Streamlit Community Cloud so the frontend does not install the backend's full ML stack.
...
| Frontend hosting | Streamlit Community Cloud | Public research UI |
| Backend hosting | Render Web Service | Public FastAPI backend |
...
| Resource | URL |
|---|---|
| Swagger UI | [https://sec-filings-rag-api.onrender.com/docs](https://sec-filings-rag-api.onrender.com/docs) |
| OpenAPI JSON | [https://sec-filings-rag-api.onrender.com/openapi.json](https://sec-filings-rag-api.onrender.com/openapi.json) |
| Health check | [https://sec-filings-rag-api.onrender.com/health](https://sec-filings-rag-api.onrender.com/health) |

### Local API

FastAPI's generated documentation is available while the local API is running:
...
### Render-compatible API container

The API container binds to `0.0.0.0` and honors the hosting platform's `PORT` environment variable, while falling back to port `8000` for local Docker execution.

The public deployment uses the same FastAPI application behind a Render Web Service. The Streamlit frontend points to that backend through `API_BASE_URL`.
...
9. **Public demo uses resource-constrained hosting**  
   The project now has a public Streamlit frontend and Render-hosted FastAPI backend. The current public backend uses free-tier compute, so cold starts, ephemeral local runtime artifacts, and memory limits can affect resource-intensive RAG requests. Local/Docker execution remains the reference environment for the full retrieval stack.
...
- production-grade cloud scaling, persistent storage, and higher-memory inference hosting;
...
The same repository supports local execution, Docker Compose, and a public split-service deployment:

- **Streamlit Community Cloud** for the interactive frontend;
- **Render** for the FastAPI backend;
- environment-based `API_BASE_URL` wiring between the two services.
...
|---|---|---|
| Streamlit UI | [Open Live Demo](https://sec-filings-rag-intelligence.streamlit.app/) | Interactive research dashboard |
| FastAPI | [Open API Docs](https://sec-filings-rag-api.onrender.com/docs) | REST API and Swagger UI |
| Health endpoint | [Check API Health](https://sec-filings-rag-api.onrender.com/health) | Deployment health verification |

### Hosted architecture

```mermaid
flowchart LR
    A[User Browser] --> B[Streamlit Community Cloud]
    B -->|HTTPS / API_BASE_URL| C[Render FastAPI]
    C --> D[SEC EDGAR Filings]
    C --> E[SEC XBRL Company Facts]
    C --> F[Query Router]
    F --> G[Deterministic Financial Path]
    F --> H[RAG Retrieval Path]
    G --> C
    H --> C
    C --> B
```
...
Streamlit Community Cloud
  ↓
https://sec-filings-rag-api.onrender.com
  ↓
FastAPI
  ├── SEC EDGAR filing metadata/documents
  ├── SEC XBRL Company Facts
  ├── deterministic financial calculations
  └── filing-text retrieval / RAG pipeline
```

The Streamlit deployment is configured with:

```toml
API_BASE_URL = "https://sec-filings-rag-api.onrender.com"
```

The Render backend keeps SEC contact identity and runtime configuration in environment variables rather than source control.

### Try these questions

Start with structured financial questions:
...
```text
What was Apple's net income in the latest fiscal year?
```

```text
Compare Apple's revenue with the previous fiscal year.
```

Then try filing-text research:
...
```text
Explain why Apple's revenue changed and calculate the growth.
```

### Public-demo note

The hosted demo is intended for portfolio review and technical demonstration. The current Render backend uses free-tier compute, so the first request can be delayed by a cold start and model-heavy RAG requests can be constrained by memory. Structured SEC/XBRL financial queries are substantially lighter. The local Docker setup remains the best environment for exercising the complete retrieval stack without free-tier hosting limits.
